...
#Source: https://github.com/vsergeev/ntgbtminer/commit/d99f735329ae753fc4f808ea3e327648eb626b66
...
asset_grid20=np.linspace(0,util(4000000000),NPOINTS) #notice that we will 1) not restrict for assets to be integers
...
xnew=utilf(policy20) #square the array to get a nonce guess

noncelist = xnew.tolist()
noncelistr = noncelist[::-1]

for i in range(0, len(noncelistr)):
    noncelistr[i] = int(noncelistr[i])
################################################################################
...
def rpc_getblocktemplate():
    try:
        return RPC_URL.getblocktemplate({'rules': ['segwit']}) 
    except ValueError:
        return {}
    print("step 1")
def rpc_submitblock(block_submission):
    return RPC_URL.submitblock(block_submission)
...
def bitcoinaddress2hash160(addr):
    ...
    table = "123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"
...
def block_mine(block_template, coinbase_message, extranonce_start, address, timeout=None, debugnonce_start=False):
    ...
    #current_block = RPC_URL.getblockcount()
...
def standalone_miner(coinbase_message, address):
    while True:
        ...
        mined_block, hash_rate = block_mine(block_template, coinbase_message, 0, address, timeout=5000)
